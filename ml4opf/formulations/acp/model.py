...
class ACPModel(OPFModel, ABC):
    """`OPFModel` for ACPPowerModel/ACOPF"""
    ...
    def evaluate_model(
        ...
        violations["pg_gap"]  = ACPViolation._reduce_violation((pred_pg - test_pg).abs(), reduction=inner_reduction)
        violations["qg_gap"]  = ACPViolation._reduce_violation((pred_qg - test_qg).abs(), reduction=inner_reduction)
        violations["vm_gap"]  = ACPViolation._reduce_violation((pred_vm - test_vm).abs(), reduction=inner_reduction)
        violations["va_gap"]  = ACPViolation._reduce_violation((pred_va - test_va).abs(), reduction=inner_reduction)
        violations["dva_gap"] = ACPViolation._reduce_violation((pred_dva - test_dva).abs(), reduction=inner_reduction)

        pred_pf, pred_pt, pred_qf, pred_qt = flows
        violations["pf_gap"] = ACPViolation._reduce_violation((pred_pf - test_pf).abs(), reduction=inner_reduction)
        violations["pt_gap"] = ACPViolation._reduce_violation((pred_pt - test_pt).abs(), reduction=inner_reduction)
        violations["qf_gap"] = ACPViolation._reduce_violation((pred_qf - test_qf).abs(), reduction=inner_reduction)
        violations["qt_gap"] = ACPViolation._reduce_violation((pred_qt - test_qt).abs(), reduction=inner_reduction)

        violations["obj_mape"] = ((pred_obj - test_obj) / test_obj).abs()

        return ACPViolation._reduce_violations(violations, reduction=reduction, dim=0)


class PerfectACPModel(ACPModel):
    def __init__(self, problem: ACPProblem):
    ...
    @staticmethod
    def load_from_checkpoint(self, path_to_folder: str, problem: ACPProblem):
